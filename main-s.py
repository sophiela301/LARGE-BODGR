...
parser.add_argument("--epoch", default=170, type=int)
...
for epoch in range(args.pretrain_epoch):
    ...
    for (group_users, group_masks, user_items, corrupt_user_items, _) in gi_data:
        ...
        _, group_embeds, _, _, _ = rec_model.pre_forward(
...
for epoch in range(args.epoch):
    ...
    for batch_data in gi_data:
        ...
        group_logits, group_embeds, scores_ug, weights, targets = rec_model(
            group_masks, user_items)
        weight_loss = rec_model.weight_loss(weights, targets)  
        ...
        # group_loss = weight_loss+group_loss 
        optimizer.zero_grad()
        group_loss.backward()
        optimizer.step()
