...
class GroupDataset(object):
    ...
    def group_dataloader(self, batch_size, test_mode=False, num_corrupt=5):
        ...
        max_len = max([len(members)
                      for members in self.group_member_dict.values()])
        users_feat = self.user_pretrain_dataloader(
            batch_size, test_mode=True)  # User features
        ...
        for group_id in range(self.num_groups):
            ...
            if len(members) < max_len:
                ...
                group_user = torch.hstack(
                    (torch.LongTensor(members), torch.zeros((max_len-len(members),)))).long()
            else:
        ...
        train_data = TensorDataset(torch.stack(all_group_users), torch.stack(all_group_mask),
                                   torch.stack(all_user_items), torch.stack(
                                       all_corrupt_user_items),
                                   torch.FloatTensor(group_feat))
        return DataLoader(train_data, batch_size=batch_size, shuffle=True)
