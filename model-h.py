...
class AttentionAggregator(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        n_high_variances = int(0.5 * variances.shape[0])
        threshold = sorted_variances[n_high_variances-1]

        high_variance_mask = (variances > threshold).float()
        high_variance_indices = high_variance_mask.nonzero(as_tuple=True)[
            0]  
        high_variance_members = torch.topk(
            weight[high_variance_indices], k=1, dim=1).indices.squeeze()  
        # [k,mask,64]
        high_variance_member_embedding = h[high_variance_indices]
        # high_variance_group_embedding = torch.zeros(
        #     high_variance_members.shape[0], 64)
        for i in range(high_variance_indices.shape[0]):
            if high_variance_indices.shape[0] == 1:
                ret[high_variance_indices[i]
                    ] = high_variance_member_embedding[i][high_variance_members]
            else:
                ret[high_variance_indices[i]
                    ] = high_variance_member_embedding[i][high_variance_members[i]]
        low_variance_mask = (variances <= threshold).float()
        low_variance_indices = low_variance_mask.nonzero(as_tuple=True)[0]
        low_variance_group_embedding = torch.sum(
            weight[low_variance_indices] * h[low_variance_indices], dim=1)
